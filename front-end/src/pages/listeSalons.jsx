...
function ListeSalons() {
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      {salonsDisponibles.length === 0 ? (
        <p className="text-center text-gray-500">Aucun salon en attente pour le moment</p>
      ) : (
        <ul className="space-y-4">
          {availableSalons.map(salon => (
            <li key={salons._id} className="p-4 border rounded flex justify-between items-center">
              <div>
                <p className="font-semibold text-lg">{salons.name}</p>
                <p className="text-sm text-gray-600">
                  Joueurs : {salons.players.length}/{salons.maxPlayers}
                </p>
                <p className="text-sm text-gray-500">Manches : {salons.maxRounds}</p>
              </div>
              <button
                className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700"
                onClick={() => console.log("Rejoindre salon", salons._id)}            
              >
                Rejoindre
              </button>
            </li>
